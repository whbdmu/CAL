...
for ci = 1:pred_classnum
    incluster = Y(find(predY == predLidx(ci))); % find(predY == predLidx(ci))返回predY中值为predLidx(ci)的 索引;  对于每个预测类别，找出属于该类别的真实标签。
%     cnub = unique(incluster);
%     inclunub = 0;
%     for cnubi = 1:length(cnub)
%         inclunub(cnubi) = length(find(incluster == cnub(cnubi)));
%     end;
    inclunub = hist(incluster, 1:max(incluster)); if isempty(inclunub) inclunub=0;end;  % 计算每个预测类别中真实标签的数量，取最大值。统计每个真实标签在 incluster 中的出现次数
    % 1:max(incluster):incluster(31*1)，前30个为3，31为1；所以1:max得到1:4的向量，统计标签1、2、3、4分别出现的次数，得[0,0,30,1]
    correnum = correnum + max(inclunub); % 将所有最大值相加，得到正确分类的样本数量，最后计算纯度。
end;
